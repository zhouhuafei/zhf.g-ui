...
module.exports = {
    ...
    output: {
        ...
        filename: `static/[name].[id].bundle.${configEnvironment.chunkhash}js`, // 出的文件名
        publicPath: `./static/`, // 静态资源引入的路径方式
        /*
        ...
        */
        chunkFilename: `static/[name].[id].chunk.${configEnvironment.chunkhash}js`,
    },
    // 插件----配置
    plugins: [
        ...
        new HtmlWebpackPlugin({
            ...
            filename: `index.html`, // 出口模板
            // 需要引入的chunk,不配置就会引入所有被CommonsChunkPlugin提取出的公共js和所有入口js,模板视图文件里js的引入顺序和chunks里的排序无关,和CommonsChunkPlugin里的顺序有关(倒叙)
            chunks: ['index', 'this-is-global-file-common', 'this-is-global-file-vendor', 'this-is-global-file-manifest'],
            minify: false, // 压缩视图模板文件
        }),
        // 插件----提取css样式到文件
        new MiniCssExtractPlugin({filename: `static/[name].${configEnvironment.contenthash}css`}),
        // 插件----压缩图片
        new ImageminPlugin({
            disable: false,
            pngquant: {
                ...
                use: [
                    {
                        ...
                        options: {
                            limit: 8192,
                            name: `static/[name].${configEnvironment.hash}[ext]`,
                        },
                    },
                ],
            },
