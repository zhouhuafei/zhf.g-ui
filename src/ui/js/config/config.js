class Config {
    constructor() {
        this.outermostContainer = window['g-outermost-container'] || 'body'; // 这里需要手动配置。最外层的容器(仅支持选择器格式，例如：'.wrap'，'#container')。
        this.outermostContainerDom = document.querySelector(this.outermostContainer); // 最外层容器的dom节点。
        this.outermostContainerMaxWidth = this.outermostContainerDom.offsetWidth; // 最外层容器的最大宽度。
        ...
        this.resetOutermostContainerMaxWidth = function () {
            this.outermostContainerMaxWidth = this.outermostContainerDom.offsetWidth; // 最外层容器的最大宽度。
        };
        this.px2remCanvas = function (px) { // canvas自适应
            if (this.isH5) { // 如果是h5，canvas才有自适应功能
                return px * this.outermostContainerMaxWidth / this.h5PsdWidth;
            } else {
                return px;
            }
        };
